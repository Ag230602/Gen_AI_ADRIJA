...
STEPS: List[Step] = [
    Step("pdf", "1) Build corpus from PDFs", "training_code/pdf_to_jsonl.py", ["--mode", "paragraph", "--min_chars", "200"]),
    Step("tok", "2) Train tokenizer", "training_code/train_tokenizer.py", []),
    Step("diag", "3) Tokenizer diagnostics", "training_code/tokenizer_diagnostics.py", []),
    Step("ret", "4) Retrieval demo", "training_code/retrieval/run_retrieval_demo.py", []),
    Step("peft", "5) Base vs Adapted (LoRA)", "training_code/base_vs_peft.py", []),
]
...
def collect_submission_sources() -> Tuple[Dict[str, Path], List[str]]:
    ...
    sources: Dict[str, Path] = {
        ...
        "code/training_code": REPO_ROOT / "training_code",
