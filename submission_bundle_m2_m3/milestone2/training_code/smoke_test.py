...
def load_split_texts_if_present(repo_root: str) -> Tuple[Optional[List[str]], Optional[List[str]], Optional[List[str]]]:
    """Load data/splits/{train,val,test}.jsonl if all exist; else return (None, None, None)."""
    ...
    return load_jsonl_texts(train_path), load_jsonl_texts(val_path), load_jsonl_texts(test_path)
...
def main() -> None:
    ...
    # Prefer explicit reproducible splits if present.
    train_texts, val_texts, test_texts = load_split_texts_if_present(repo_root)
    if train_texts is None:
        ...
        train_texts, val_texts, test_texts = split_train_val_test(
            texts, train_fraction=0.8, val_fraction=0.1, test_fraction=0.1, seed=cfg.seed
        )
    ...
    base_test_ids = encode_texts_limited(tokenizer, test_texts, max_tokens=max(10_000, cfg.max_train_tokens // 5))
    ...
    test_repeat = max(1, train_repeat // 4)
    ...
    if len(test_ds) == 0:
        raise RuntimeError("Test dataset produced 0 samples. Try reducing seq_len or increasing repeat_corpus.")
    ...
    test_batch_size = min(cfg.batch_size, max(1, len(test_ds)))
    ...
    with open(loss_log_path, "w", encoding="utf-8") as f:
        f.write("step\ttrain_loss\tval_loss\ttest_loss\n")
        ...
        while global_step < cfg.max_steps:
            for batch in train_loader:
                ...
                if global_step % cfg.eval_every == 0:
                    ...
                    last_test_loss = eval_loss(model, test_loader, device=device, max_batches=5)
                ...
                print(
                    f"step={global_step} train_loss={train_loss:.4f} val_loss={val_loss:.4f} test_loss={test_loss:.4f}"
                )
                f.write(f"{global_step}\t{train_loss:.6f}\t{val_loss:.6f}\t{test_loss:.6f}\n")
    ...
    test_x = [s for s, v in zip(steps, test_losses) if not math.isnan(v)]
    test_y = [v for v in test_losses if not math.isnan(v)]
    if test_x:
