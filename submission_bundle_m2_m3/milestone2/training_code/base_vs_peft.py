...
@dataclass
class LoRAConfig:
    target_modules: Tuple[str, ...] = ("out_proj", "linear1", "linear2", "lm_head")
    rank: int = 8
    alpha: int = 16
    dropout: float = 0.05
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, cfg: ExperimentConfig) -> Dict[str, float]:
    criterion = nn.CrossEntropyLoss()
...
def main() -> None:
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("|---|---:|---:|---:|---:|\n")
        f.write(f"| Base | {base_params['trainable']} | {base_params['total']} | {base_loss:.4f} | {base_ppl:.4f} |\n")
        f.write(f"| Adapted (LoRA) | {lora_params['trainable']} | {lora_params['total']} | {lora_loss:.4f} | {lora_ppl:.4f} |\n")
