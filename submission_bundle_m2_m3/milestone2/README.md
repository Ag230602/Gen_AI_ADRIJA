# Milestone 2 — StormCare RAG (Track C)
...
The comparison uses the same task, held-out split, metrics, and evaluation procedure for base vs LoRA-adapted systems.
...
Optional reproducible splits (recommended)

To create fixed train/val/test splits (shuffled with `seed=42`) and save them under `data/splits/`:
...
Train split used:
- preferred: use the fixed JSONL split files under `data/splits/` (seed=42)
- fallback: document-level split with fixed seed (`seed=42`): 80% train / 10% val / 10% test
...
Outputs:
- `results/loss_log.txt` (tab-separated: step, train_loss, val_loss, test_loss)
